...
function checkDeleteButton(object) {
   var len = 0
...
const getname = (user) => {
   ...
   if (user.data) {
       if (!user.data.firstname && !user.data.lastname) {
           text = user.data.email.split('@')[0];
       } else if (user.data.nameshow == 'rev') {
           text = user.data.lastname + " " + user.data.firstname;
       } else {
...
const sortJsonArray =(object) =>{
   const orderedArray = [];
   object.map(unordered=>{
      let ordered={}
      Object.keys(unordered).sort().forEach(function(key) {
         ordered[key] = unordered[key];
       });
       orderedArray.push(ordered);
   })

    return orderedArray;
}

const CheckObjectPresentInArray =(array,obj)=>{
   var count=0
   array.map(obj1 =>{
      if(JSON.stringify(sortJson(obj1)) === JSON.stringify(sortJson(obj))){
         count+=1
      }
   })
...
const conditionarray=(data,permission)=>{
   console.log(data);
