const verficationCodeFirstTime = (arg: any) => {
...
const forgotPasswordSendVerificationCode= (arg: any) => {
  // const base64data = Buffer.from(arg.email).toString('base64')
  return {
    from: "noreply@appypieinc.com",
    to: arg.email,
...
const emailAfterComment = (arg: any) => {

  const name: any = (arg.firstname !== undefined && arg.firstname.length > 0 && arg.lastname !== undefined && arg.lastname.length > 0) ? arg.firstname[0].toUpperCase() + arg.firstname.slice(1) + " " + arg.lastname[0].toUpperCase() + arg.lastname.slice(1) : (arg.firstname !== undefined && arg.firstname.length > 0) ? arg.firstname[0].toUpperCase() + arg.firstname.slice(1) : arg.email.split('@')[0][0].toUpperCase() + arg.email.split('@')[0].slice(1)
